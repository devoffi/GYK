...
const Pricing = ({ selectedPlan, onPlanSelect , name, email, phone_number, username, password }) => {
  ...
  const handleSubmit = async () => {
    try{
      if(referal.length > 2){
        const response = await axios.post('https://mdm.prabhaktech.com/api/validate-referralcode', {"referral_code": referal});
  ...
  const createUser = async (userData) => {
    const response = await axios.post('https://mdm.prabhaktech.com/api/create-user', userData);
  ...
  const renderCards = () => {
    ...
    return plans.map((plan) => (
      <div key={plan.id} className={`relative md:w-[50%] border rounded-xl p-8 ${plan.id === 'Premium' ? 'bg-blue-900/90' : 'bg-white'} hover:shadow-xl transition-shadow duration-300 mb-10`}>
        {plan.id === 'Premium' && (
        ...
        <div className="mb-6 border-b">
          <h2 className={`text-xl font-semibold ${plan.id === 'Premium' ? 'text-white' : 'text-gray-700'} mb-2 flex gap-2`}>
            {plan.icon}
            {plan.title}
          </h2>
          <p className={`text-base font-light md:text-xl ${plan.id === 'Premium' ? 'text-white' : 'text-gray-900'}`}>{plan.description}</p>
        </div>
        <div className="text-center my-2">
          <p className={`text-3xl font-medium flex items-end ${plan.id === 'Premium' ? 'text-white' : 'text-gray-800'}`}>
          <span className=' px-2 text-[18px] flex justify-end'>{currency}</span>{isMonthly ? plan.priceMonthly : plan.priceYearly}
            <span className={`text-lg pb-1 ${plan.id === 'Premium' ? 'text-white' : 'text-gray-700'}`}>/{isMonthly ? 'month' : 'year'}</span>
          </p>
          {isMonthly ? (
            <p className={`text-start px-1 font-medium ${plan.id === 'Premium' ? 'text-white' : 'text-gray-500'} pb-3`}>
          ...
          <button
            ...
            className={`hover:text-white hover:bg-viridianGreen hover:font-[400] font-[500] w-full py-[8px] border-[2px] border-viridianGreen border-solid rounded-md ${plan.id === 'Premium' ? 'bg-white text-black' : 'bg-transparent text-black'} sm:text-md text-xl`}
          >
            Buy now
          </button>
        </div>
...
console.log("Exchange Rate:", exchangeRate);
  ...
  return (
    <PayPalScriptProvider options={{
      "client-id": "ASeTLjejgCUSUlXzY65o5s6iHaUTTejxLc_kKbRicH-iClmKXjlCmsvjVQ-pIkO3Dz6Xidffl-0t92-v", 
      currency: currency ,
    }}>
        ...
        <div className="gap-4">
          ...
          <div>
            ...
            <div className='flex gap-5'>
              ...
              />
              <button className=' my-2  px-8 bg-viridianGreen text-white rounded' onClick={handleSubmit}>Verify</button>
            </div>
          ...
          {selectedPlan && (
            <div className="mt-2 w-full flex flex-col justify-center items-center">
              <h3 className="text-lg font-medium">You selected: <span className='inline font-bold'>{selectedPlan} Plan { amount}</span></h3>
              <div className="mt-4">
                { currency === "INR" ? (
                   <button 
